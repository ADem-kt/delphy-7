...
Cr:array[1..18]of integer;//для вывода на график отдельных насосов
ss: String;
s,si,su,su2,su3,SF,s1:TLineSeries;

begin
tx:=0.01;//шаг построения по х
ty:=0.01;//шаг построения по y
   //замена . на ,
...
for i := StringGrid2.FixedCols to Pred(StringGrid2.ColCount) do
    ...
    if Trim(StringGrid2.Cells[i, j]) = '' then
       begin
        ShowMessage('Ошибка: Обнаружена пустая ячейка!');
        Exit;
       end;
       end;
...
for i := StringGrid2.FixedCols to Pred(StringGrid2.ColCount)-1 do
    ...
    if x=0 then
       begin
        ShowMessage('Ошибка:Неверное значение параметра!');
        Exit;
       end;
       end;


//считывае количество насосов в группе
...
//изменяем шаг построения графиков
       //находим правый край графика для определения шага
       xRight:=0;
        for i:=2 to n+1 do
        ...
        xRight3:=xRight3+sqrt(Br[1,i]/Br[2,i]);
         ...
         if xRight>=25000 then tx:=120;
           //находим yTop для определения шага
       yTop:=0;
       for i:=2 to n+1 do
       ...
       if Br[1,i]>=yTop3 then yTop3:=Br[1,i];
        ...
        if (yTop+yTop2+yTop3>=25000) then ty:=100;
         ...
         //находим yTop для финального графика
       yTop:=10000;
       for i:=2 to n+1 do
       if Br[1,i]<=yTop then yTop:=Br[1,i];
       if n2<>0 then
     ...
     // удаляем серии
       ...
       //создали серии для Суммарного графика и для графика системы
...
begin
 ...
 begin
  ...
  begin
       ...
       StringGrid1.Cells[0,r]:=floatToStr(Trunc(G2[i-n-1,e*(r-1)+1]));
       StringGrid1.Cells[1,r]:=floatToStr(e*(r-1)*ty+1);
       end;
        StringGrid1.Cells[0,10]:='0';
       StringGrid1.Cells[1,10]:=FloatToStr(Br[1,i]);
 ...
 begin
  ...
  begin
       ...
       e:=trunc(ir/9);
       for r:=1 to 10 do begin
       StringGrid1.Cells[0,r]:=floatToStr(Trunc(G3[i-n-n2-1,e*(r-1)+1]));
       StringGrid1.Cells[1,r]:=floatToStr(e*(r-1)*ty+1);
       end;
       StringGrid1.Cells[0,10]:='0';
       StringGrid1.Cells[1,10]:=FloatToStr(Br[1,i]);
         ...
         //Суммарынй график через суммирование G1
       ir:=0;
       y:=0;
       repeat
       x:=0;
       ...
       //su.Name:= 'G1';
        ...
        //перевод в массив с шаuом по х 1 группа
        xRight:=0;
        for i:=2 to n+1 do
        xRight:=xRight+sqrt(Br[1,i]/Br[2,i]);

        for j:=1 to trunc(xRight/tx-1) do begin
        x:=j*tx;
        x1:=0;
        x2:=0;
        y1:=0;
       ...
       //su2.Name := 'G2';
               ...
               //перевод в массив с шаuом по х 2 группа
        xRight:=0;
        for i:=n+2 to n+n2+1 do
        xRight:=xRight+sqrt(Br[1,i]/Br[2,i]);
        for j:=1 to trunc(xRight/tx-1) do begin
       ...
       //su3.Name := 'G3';
        ...
        //перевод в массив с шагом по х 3 группа
        xRight:=0;
        for i:=n+n2+2 to n+n2+n3+1 do
        xRight:=xRight+sqrt(Br[1,i]/Br[2,i]);
        for j:=1 to trunc(xRight/tx-1) do begin
       ...
       xRight:=0;
        ...
        xRight3:=xRight3+sqrt(Br[1,i]/Br[2,i]);
         ...
         xRight:=trunc(xRight/tx);
                //Итоговый суммарынй график через суммирование Gx
       j:=0;
       repeat
       j:=j+1;
       for i:=1 to 3 do
       ...
       e:=Trunc(x/9);
       for r:=1 to 10 do begin
       StringGrid1.Cells[(n+n2+n3)*2+1,r]:=floatToStr(Trunc((Br[1][1]+Br[2][1]*sqr(e*r))*10)/10);
       StringGrid1.Cells[(n+n2+n3)*2,r]:=floatToStr(e*r);
       end;
       ...
       si.Name := 'Sistema';
           ...
           {Находим пересечение суммарного графика и графика системы}
       x:=0;
       y:=0;
       ir:=0;
        repeat
        ...
        Q:=x;
        H:=y;
       label8.Caption:=floatToStr(trunc(Q*100)/100)+' м3/ч';
       label11.Caption:=floatToStr(trunc(H*100)/100)+' м';
        if Sr[ir]=0 then
        begin
        label8.Caption:='-';
        label11.Caption:='-';
        end;
        if (H-ty*25)>yTop+yTop2+yTop3 then
        begin
        label8.Caption:='-';
        label11.Caption:='-';
        end;
        ...
        With Chart1.LeftAxis do
          ...
          //новая таблица координатами графика
          if combobox3.ItemIndex= n+n2+n3  then
          begin
StringGrid1.ColCount:=2*(n+n2+n3);
StringGrid1.RowCount:=11;
For i:=1 to n+n2+n3 do begin
StringGrid1.Cells[i*2-2,0]:='Q'+intToStr(i)+' м3/ч';
StringGrid1.Cells[i*2-1,0]:='H'+intToStr(i)+' м';
StringGrid1.Cells[(n+n2+n3)*2,0]:='Qсист м3/ч';
StringGrid1.Cells[(n+n2+n3)*2+1,0]:='Hсист м';
end;
StringGrid1.Height:=281;//StringGrid1.RowCount*27;
StringGrid1.ColCount:=(n+n2+n3)*2+2;
...
StringGrid1.RowCount:=11;
StringGrid1.Cells[0,0]:='Q м3/ч';
StringGrid1.Cells[1,0]:='H м';
...
n2:=StrToInt(Edit1.Text);
n3:=StrToInt(Edit2.Text);
//довавляем в КомбоБокс номера насосов
...
if n+n2+n3>=2 then
ComboBox3.Items.Add('Все насосы');


//Очищаем Развернутую схему
image20.Picture:=nil;
...
//новая таблица с паметрами
StringGrid2.ColCount:=4;
StringGrid2.Cells[1,0]:='H';
StringGrid2.Cells[2,0]:='S';
StringGrid2.Cells[3,0]:='Граф.(0/1)';
StringGrid2.Cells[0,1]:='Система';
for i:=1 to n+n2+n3 do
...
a:=75;//расстояние от насоса до задвижки на всасе
b:=60;//расстояние от насоса до задвижки и обратного клапана на напоре
c:=100;//расстояние между насосами в группе
d:=350;//расстояние между группами насосов
x0:=125;//начальная точка по ширине
y0:=50;//начальная точка по высоте
a1:=110;//расстояние от насоса до общейго коллектора на всасе
b1:=210;//расстояние от насоса до общейго коллектора на напоре
a2:=100;//расстояние от насоса до манометра на всасе
b2:=150;//расстояние от насоса до манометра на напоре
b3:=175;//расстояние от насоса до расходомера на напоре
//Передвигаем схему в зависимоти от кол-ва насосов
//по х
if n3=0 then x0:=285;
if n2=0 then x0:=500;
...
//Делаем видимыми image 1 группы
    for i:=1 to n do
    begin
   if i=1 then
   begin
...
//Делаем видимыми image 2 группы
for i:=1 to n2 do
    begin
   if i=1 then
   begin
...
//нарисовали рисунок при открытии формы
procedure TForm1.FormCreate(Sender: TObject);
var i,n,n2,n3,x0,y0:integer;
begin
Memo1.Text:='Краткое описание насоса';
Button3.Click;
//рисуем оси
x0:=0;
y0:=50;
...
//считываем кол-во насосов
n:=StrToInt(Edit5.Text);
n2:=StrToInt(Edit1.Text);
n3:=StrToInt(Edit2.Text);

                                              //добовляем стандартные насосы
ComboBox1.Items.Add('ЦВН-8');
ComboBox1.Items.Add('ГЦН-195М');
ComboBox1.Items.Add('ГЦЭН-310');
ComboBox1.Items.Add('ГЦН-317');
ComboBox1.Items.Add('ПЭА 1650-75');
ComboBox1.Items.Add('ПЭА 250-75');
ComboBox1.Items.Add('ПТА 3750-75');
ComboBox1.Items.Add('ПТА 3800-20');
ComboBox1.Items.Add('КсА 1500-240-2');
ComboBox1.Items.Add('КсВА 1500-120');
ComboBox1.Items.Add('КсВ 1000-220');
ComboBox1.Items.Add('КсВА 900-180');
ComboBox1.Items.Add('КсВА 630-125');
ComboBox1.Items.Add('КсВА 500-220');
ComboBox1.Items.Add('КсВА 360-160');
ComboBox1.Items.Add('{Пустой насос}');
...
//процедура для рисования новых ветвей наоссов
...
i:=i-1;
until i<n+1 ;                                 //вводим параметры в талицу для конкретного насоса
...
//прочерчиваем линии вверх, которые удоляются
pen.Color:=ClGreen;
pen.Width:=4;
MoveTo(x0,y0+25*(n-2));
Lineto(x0,y0-25);
...
LineTo(x0+100,y0-25);
Edit5.Text:=intTostr(n);
//стираем маленькие точечки
MoveTo(x0,25*(n+1));
pen.Color:=Clwhite;
pen.Width:=4;
LineTo(x0,25*(n)+20);
//Заново прочерчиваем начальный насос
pen.Color:=Clgreen;
MoveTo(x0,y0);
LineTo(x0,y0-25);
MoveTo(x0+100,y0-25);
...
n:=n+1;                                      //вводим параметры в талицу для конкретного насоса
if combobox1.ItemIndex=0 then begin
StringGrid2.Cells[1,n+1]:='265';
StringGrid2.Cells[2,n+1]:='0,0000009';
end;
...
i:=i-1;
until i<n+n2+1;
n:=n+n2;                                  //вводим параметры в талицу для конкретного насоса
if combobox1.ItemIndex=0 then begin
StringGrid2.Cells[1,n+1]:='265';
StringGrid2.Cells[2,n+1]:='0,0000009';
end;
...
n:=n+n2+1;                                 //вводим параметры в талицу для конкретного насоса
if combobox1.ItemIndex=0 then begin
StringGrid2.Cells[1,n+1]:='265';
StringGrid2.Cells[2,n+1]:='0,0000009';
end;
...
Button3.Click;
                              ...
                              //вводим параметры в талицу для конкретного насоса
n:=n+n2+n3;
if combobox1.ItemIndex=0 then begin
StringGrid2.Cells[1,n+1]:='265';
StringGrid2.Cells[2,n+1]:='0,0000009';
...
begin
if combobox1.ItemIndex=0 then begin
Memo1.Text:='Главный циркуляционный насос ЦВН-8 предназначен для создания принудительной циркуляции теплоносителя в контуре многократной принудительной циркуляции ( К М П Ц ) реакторных установок типа РБМК-1000';
...
if combobox1.ItemIndex=1 then begin
Memo1.Text:='Главный циркуляционный насосГЦН-195М в сейсмостойком исполнении предназначен для создания циркуляции теплоносителя в первом контуре реакторных установок АЭС с реактором ВВЭР-1000';
...
if combobox1.ItemIndex=2 then begin
Memo1.Text:='Главный циркуляционный насос ГЦЭН-310 предназначен для создания циркуляции теплоносителя в первом'+#13+#10+'контуре АЭС с ректором ВВЭР-440.Насосы ГЦЭН-310 эксплуатируются на энергоблоках № 3 и 4 Нововоронежской АЭС и № 1 и 2 Кольской АЭС со дня ввода их в эксплуатацию.';
Image111.Visible:=false;
Image112.Visible:=false;
...
if combobox1.ItemIndex=3 then begin
Memo1.Text:='Главный циркуляционный насос ГЦН-317 предназначен для создания циркуляции теплоносителя в первом контуре АЭС с реактором ВВЭР-440. Насосы эксплуатируются на энергоблоках № 3 и 4 Кольской АЭС.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
...
if combobox1.ItemIndex=4 then begin
Memo1.Text:='Насос ПЭА 1650-75 применяется для подачи питательной воды из деаэратора в контур многократной' +#13+#10+' принудительной циркуляции энергоблоков АЭС с реактором РБМК-1000.Насос центробежный, горизонтальный, секционный, трехступенчатый, с односторонним расположением рабочих колес.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
if combobox1.ItemIndex=5 then begin
Memo1.Text:='Насос ПЭА 250-75 применяется для работы в качестве аварийного питательного насоса на энергоблоках' +#13+#10+' АЭС с реактором РБМК-1000.Температура питательной воды на входе в насос должна быть не выше 170°С. Насос допускает длительную эксплуатацию в рабочей части характеристики.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
if combobox1.ItemIndex=6 then begin
Memo1.Text:='Насос ПТА 3750-75 применяется для подачи питательной воды в парогенераторы энергоблоков АЭС с реакторами ВВЭР-1000.Привод насоса осуществляется непосредственно от конденсационной паровой турбины ОК-12А.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
if combobox1.ItemIndex=7 then begin
Memo1.Text:='Насос ПТА 3800-20 применяется в качестве бустерного для обеспечения бескавитационной работы питательного насоса ПТА3750-75. Привод насоса осуществляется от конденсационной паровой турбины ОК-12А через понижающий редуктор.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
if combobox1.ItemIndex=8 then begin
Memo1.Text:='Насосы КсА 1500-240-2 (ЦН1500-240) ,КсА1500-240-2а применяются для подачи конденсата в деаэратор на АЭС с реакторами РБМК-1000 и ВВЭР-1000.Насосы центробежные, горизонтальные, одноступенчатые с рабочим колесом двустороннего входа.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
if combobox1.ItemIndex=9 then begin
Memo1.Text:='Насос КсВА 1500-120 применяется для подачи конденсата отработанного пара из конденсаторов турбоустановок на обессоливающие фильтры на АЭС с реакторами РБМК-1000 и ВВЭР-1000.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
Memo1.Text:='Насос КсВ 1000-220 применяется для подачи конденсата отработанного пара температурой до 70 °С из конденсатора турбоустановок К-500-60/1500(5-йэнергоблок Нововоронежской АЭС) в деаэратор.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
Memo1.Text:='Насосы КсВА 900-180 и КсВА 1000-190 применяются для перекачивания конденсата греющего и отработанного пара стационарных паровых турбин энергетических блоков атомных электростанций с реактором ВВЭР-1000.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
Memo1.Text:='Насос КсВА 630-125 применяется для подачи конденсата (дренажа) греющего пара в тракт основного конденсата после подогревателя низкого давления ПНД-3 турбоустановок K-500-60/1500 на АЭС с реактором ВВЭР-1000.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
Memo1.Text:='Насос КсВА 500-220 применяется для перекачивания конденсата пара турбоустановок К-220-44 в деаэраторы на АЭС с реактором ВВЭР-440.Насос центробежный, вертикальный, двухкорпусной,' +#13+#10+'  секционного типа, с предвключенным колесом, трехступенчатый, с односторонним расположением рабочих колес и разгрузочным барабаном.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
Memo1.Text:='Насос КсВА 360-160 применяется для подачи конденсата (дренажа) греющего пара в тракт основного конденсата после ПНД-1 турбоустановокК-500-60/1500на АЭС с реактором ВВЭР-1000.';
Image111.Visible:=false;
Image112.Visible:=false;
Image113.Visible:=false;
Image114.Visible:=false;
...
var n,n2,n3:integer;
begin
Label30.Caption:='Изучите последовательное соединение насосов';
Panel1.Visible:=true;
Panel1.Left:=0;
Panel1.Top:=0;
...
//Делаем неактивными кнопки добавления удаления насосов
...
var n,n2,n3:integer;
begin
Label30.Caption:='Изучите параллельное соединение насосов';
Panel1.Visible:=true;
Panel1.Left:=0;
Panel1.Top:=0;
Panel1.Height:=Form1.Height;
...
var n,n2,n3:integer;
begin
Label30.Caption:='Изучите взаимодействие насосов в произвольной схеме';
Panel1.Visible:=true;
Panel1.Left:=0;
Panel1.Top:=0;
Panel1.Height:=Form1.Height;
...
if n2<>0 then Button11.Click;
if n<>0 then Button15.Click;
//Делаем активными кнопки добавления удаления насосов
Button2.Enabled:=true;
Button5.Enabled:=true;
Button6.Enabled:=true;
Button7.Enabled:=true;
...
var r:integer;
begin
r:=MessageDlg('Программа быдет закрыта',mtWarning,mbOKCancel,0);
